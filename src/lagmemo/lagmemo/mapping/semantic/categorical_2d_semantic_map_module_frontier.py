...
class Categorical2DSemanticMapModule(nn.Module):
    ...
    def _update_local_map_and_pose(  # noqa: C901
        ...
        # 如果depth中值为0的区域大于1/4，则对空洞区域进行填补，wxl
        if (depth == 0).sum() > depth.numel() // 4:
            fill_v = torch.mean(depth[(depth > 0) & (depth < self.max_depth)]) 
            depth[depth == 0] = fill_v if fill_v > 0 else self.min_depth
        ...
        # current_map[:, MC.EXPLORED_MAP] = prev_map[:, MC.EXPLORED_MAP]
        ...
        for e in range(batch_size):
            ...
            try:
                ...
                obstacle_map = np.array(current_map[e,MC.OBSTACLE_MAP].cpu())
                explored_map = np.array(current_map[e,MC.EXPLORED_MAP].cpu())
                ...
                if len(contours) > 1:
                    ...
                    for idx, cnt in enumerate(contours):
                        ...
                        if dist >= 0:
                            ...
                            min_dist = abs(dist)
                            best_idx = idx
                    new_area = np.zeros_like(explored_map, dtype=np.uint8)
                    cv2.drawContours(new_area, contours, best_idx, 1, -1)  # type: ignore
                    explored_map = new_area.astype(bool)
                ...
                current_map[e,MC.EXPLORED_MAP] = torch.from_numpy(explored_map.astype(np.uint8)).to(current_map.device)
                # current_map[e,MC.EXPLORED_MAP][explored_map]=1
