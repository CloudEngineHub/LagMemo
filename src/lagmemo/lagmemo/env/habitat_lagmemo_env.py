...
import lagmemo
from lagmemo.perception.constants import (
    HM3DtoCOCOIndoor,
...
class HabitatGoatEnv(HabitatEnv):
    ...
    def init_perception_module(self, vocabulary=None):
        from lagmemo.perception.detection.detic.detic_perception import (
            DeticPerception,
        )
        ...
        # from lagmemo.perception.detection.grounded_sam.ram_perception import RAMPerception
    ...
    def _preprocess_obs(
        ...
        obs = lagmemo.core.interfaces.Observations(
            rgb=habitat_obs["rgb"],
            depth=depth,
    ...
    def _preprocess_goals(self, tasks, habitat_obs):
        ...
        for i,goal_v in enumerate(goals):
            # import ipdb; ipdb.set_trace()
            goal_v["semantic_id"] = all_ovon_categories.index("_".join(goal_v["category"].split(" "))) + 1
            
            if goal_v["image"] is not None and tasks[i][1]=='image':
                goal_v["type"] = "imagenav"
            elif goal_v["description"]:
                goal_v["type"] = "languagenav"
            else:
    ...
    def _preprocess_action(self, action: lagmemo.core.interfaces.Action) -> int:

        if type(action) == int:
            return action

        discrete_action = cast(
            lagmemo.core.interfaces.DiscreteNavigationAction, action
        )
