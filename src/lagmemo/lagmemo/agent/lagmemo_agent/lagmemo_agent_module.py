...
class GoatAgentModule(nn.Module):
    ...
    def forward(
        ...
        # wxl
        # seq_map_features shape: (batch_size, sequence_length, 2 * MC.NON_SEM_CHANNELS + num_sem_categories, M, M)
        # map_features shape: (batch_size*sequence_length, 2 * MC.NON_SEM_CHANNELS + num_sem_categories, M, M)
        ...
        # frontier_img = Image.fromarray(frontier_array.astype(np.uint8))
